			...
			<section className="border-stroke-soft-100 border-b">
				<div className="mx-auto max-w-5xl border-stroke-soft-100 border-r border-l">
					<div className="flex items-center justify-between border-stroke-soft-100 border-b px-10 py-4">
						...
						<span className="text-sm text-text-sub-600">/ CATCH-ALL CHECKER</span>
					</div>
					<div className="px-10 py-16 text-center">
						...
						<p className="mx-auto mt-6 max-w-xl text-lg text-text-sub-600">
							Check if a domain is configured as catch-all (accept-all). SMTP-based
							detection with detailed analysis.
						</p>
					</div>
				</div>
			</section>
									...
									>
										<div className="flex items-center justify-between">
											<div>
												...
												<p
													className={`mt-1 font-semibold text-2xl ${
														result.isCatchAll ? "text-yellow-600" : "text-green-600"
													}`}
												>
													{result.isCatchAll ? "Catch-All Domain" : "Not Catch-All"}
												</p>
											...
											<div className="flex h-16 w-16 items-center justify-center">
												<Icon
													name={result.isCatchAll ? "alert-triangle" : "check-circle"}
													className={`h-12 w-12 ${
														result.isCatchAll ? "text-yellow-600" : "text-green-600"
													}`}
												/>
									...
									{result.smtpResponse && (
										<div className="rounded-xl border border-stroke-soft-100 bg-white p-6">
											...
											<div className="max-h-40 overflow-auto rounded-lg bg-bg-weak-50 px-4 py-3">
												<p className="font-mono text-xs text-text-strong-950">
													{result.smtpResponse}
												</p>
											</div>
										</div>
										...
										<div className="rounded-xl border border-stroke-soft-100 bg-white p-6">
											...
											<ul className="space-y-2">
												{result.implications.map((imp, i) => (
													<li key={i} className="flex items-start gap-2 text-sm text-text-sub-600">

...
const Faq = () => {
	...
	return (
		<>
			...
			<section
				className="border-stroke-soft-100 border-t border-b"
				aria-labelledby="faq-heading"
			>
				<div className="mx-auto max-w-5xl border-stroke-soft-100 md:border-r md:border-l">
					...
					<div className="flex-1">
						{faqCategories.map((category, categoryIndex) => (
							<div key={categoryIndex} className="flex flex-col lg:flex-row">
								<div
									className={`w-full border-stroke-soft-100 lg:w-3xl lg:border-r ${categoryIndex !== faqCategories.length - 1 ? "border-b" : ""}`}
								>
									<h2
								...
								<Accordion.Root type="multiple" className="w-full">
									{category.questions.map((faq, questionIndex) => {
										...
										return (
											<Accordion.Item
												className={`rounded-none border-stroke-soft-100 ${!isVeryLastItem ? "border-b" : ""} bg-transparent p-0! ring-0 hover:bg-transparent has-[:focus-visible]:bg-transparent data-[state=open]:bg-transparent`}
												value={itemValue}
												key={questionIndex}
											>

...
const BulkImportPage = () => {
  ...
  const [importResult, setImportResult] = useState<{ contacts: BulkImportResult; subscriptions: BulkAddResult } | null>(null);
  ...
  const parseCSV = (csvText: string): BulkImportContact[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const contact: BulkImportContact = {
        ...
        firstName: row.firstname || row.first_name || row.first || row.fname || "",
  ...
  const handleBack = () => {
    router.push(`/${orgSlug}/topics/${topicId}`);
  };
  ...
  return (
    <div className="mx-auto max-w-3xl pt-10 pb-8">
      <Button.Root onClick={handleBack} variant="neutral" mode="stroke" size="xxsmall">
        <Button.Icon>
          <Icon name="chevron-left" className="h-4 w-4" />
        </Button.Icon>
        Back
      ...
      <div className="flex w-full items-center justify-between border-stroke-soft-200 border-b border-dashed pt-6 pb-6">
        <div>
          <h1 className="font-medium text-title-h5 leading-8">Bulk Import Contacts</h1>
      ...
      <div className="space-y-3">
        <div
          {...getRootProps()}
          className={`flex cursor-pointer flex-col items-center justify-center gap-4 rounded-lg border border-stroke-sub-100 border-dashed p-8 transition-colors duration-200 ${isDragActive ? "border-blue-400 bg-blue-50" : "border-stroke-sub-300 hover:border-stroke-sub-400"
            }`}
        >
          <input {...getInputProps()} />
          ...
          <div className="space-y-2 text-center">
            <div className="font-medium text-sm text-text-strong-950">
              {isDragActive ? "Drop the CSV file here..." : "Choose a CSV file or drag & drop it here."}
            </div>
            <div className="text-text-sub-600 text-xs">CSV format only, up to 1000 rows.</div>
          </div>
        </div>
        ...
        {csvData.length > 0 && !importResult && (
          <div className="mt-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-lg">Preview ({csvData.length} contacts)</h3>
            ...
            {validationErrors.length > 0 && (
              <div className="rounded-lg border border-red-200 bg-red-50 p-4">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="font-medium text-red-800">{validationErrors.length} validation errors</span>
                </div>
              </div>
            )}
            ...
            <div className="max-h-64 overflow-auto rounded-lg border border-stroke-soft-200">
              {csvData.slice(0, 10).map((contact, i) => (
                <div key={i} className="flex items-center justify-between border-b border-stroke-soft-200 p-3 last:border-b-0">
          ...
          <div className="mt-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Icon name="check-circle" className="h-5 w-5 text-success-base" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-stroke-soft-200 p-6">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="font-medium text-sm text-success-base">Created</span>
                </div>
                <div className="font-bold text-2xl text-success-base">{importResult.contacts.created}</div>
              </div>
              <div className="rounded-xl border border-stroke-soft-200 p-6">
                <div className="mb-2 flex items-center gap-2">
                  <Icon name="minus-circle" className="h-4 w-4 text-text-sub-600" />
                  <span className="font-medium text-sm text-text-sub-600">Skipped</span>
                </div>
                <div className="font-bold text-2xl text-text-sub-600">{importResult.contacts.skipped}</div>
              </div>
            </div>
          </div>
        )}

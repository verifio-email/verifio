...
function getDeviceFingerprint(headers: Record<string, string | undefined>): string {
  const components = [
    headers["user-agent"] || "",
...
export const authRateLimiter = new Elysia({ name: "auth-rate-limiter" })
  .derive({ as: "scoped" }, async ({ headers, path, set, request }) => {
    ...
    if (request.method === "POST" && (path.includes("sign-in") || path.includes("sign-up"))) {
      try {
    ...
    const results: { key: string; allowed: boolean; remaining: number; resetAt: number }[] = [];

    // 1. IP-based rate limit
    const ipKey = `ip:${ip}`;
    const ipResult = checkRateLimit(ipKey, RATE_LIMITS.ip.maxRequests, RATE_LIMITS.ip.windowMs);
    results.push({ key: "ip", ...ipResult });

    // 2. Fingerprint-based rate limit
    const fpKey = `fp:${fingerprint}`;
    const fpResult = checkRateLimit(fpKey, RATE_LIMITS.fingerprint.maxRequests, RATE_LIMITS.fingerprint.windowMs);
    results.push({ key: "fingerprint", ...fpResult });

    // 3. Email-based rate limit (if email available)
    if (email) {
      const emailKey = `email:${email}`;
      const emailResult = checkRateLimit(emailKey, RATE_LIMITS.email.maxRequests, RATE_LIMITS.email.windowMs);
    ...
    if (path.includes("sign-up")) {
      ...
      const signupResult = checkRateLimit(signupKey, RATE_LIMITS.signup.maxRequests, RATE_LIMITS.signup.windowMs);
      ...
      const resetResult = checkRateLimit(resetKey, RATE_LIMITS.passwordReset.maxRequests, RATE_LIMITS.passwordReset.windowMs);
    ...
    if (blockedResult) {
      ...
      logger.warn({ ip, fingerprint, email, blockedBy: blockedResult.key, retryAfter }, "Auth rate limit exceeded");
      return {

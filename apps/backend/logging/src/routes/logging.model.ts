...
export namespace LoggingModel {
  ...
  export const logBody = t.Object({
    ...
    // Service Context
    service: t.String({
      description: "Service name: 'verify', 'api-key', 'auth', 'workflow', 'upload'"
    }),
    ...
    resource_id: t.Optional(t.String({ description: "ID or value of the resource" })),

    // Result
    status: t.String({ description: "'success', 'failed', or 'error'" }),
    result: t.Optional(t.String({ description: "Detailed result" })),
    error_message: t.Optional(t.String({ description: "Error message if failed" })),

    // Metrics
    credits_used: t.Optional(t.Number({ description: "Credits consumed" })),
    duration_ms: t.Optional(t.Number({ description: "Request duration in ms" })),
